...
import AINoteTaker from './AINoteTaker.jsx';
...
function NoteEditor({ note, onSaved, onCancel, isAdmin }) {
  ...
  const handleLoadPrevious = async () => {
    if (form.subjective || form.objective || form.assessment || form.plan) {
      if (!confirm('Replace current content with the previous note?')) return;
    }
    setLoadingPrev(true);
    try {
      const prev = await api.get(`/notes/previous/${note.client_id}`);
      setForm({
      ...
      alert('No previous finalized notes found for this client.');
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (note?.id) {
        ...
        await api.post('/notes', { ...payload, appointment_id: note.appointment_id, client_id: note.client_id });
      }
      onSaved();
    ...
    <div className="space-y-4">
      ...
      {showAI ? (
        ...
        <button onClick={() => setShowAI(true)}
          className="w-full border border-dashed border-gray-300 rounded-xl p-3 bg-gray-50/50 hover:bg-gray-100/50 transition-colors flex items-center justify-center gap-2 text-sm font-medium text-gray-600 hover:text-gray-900">
          <svg className="w-4 h-4 text-brand-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-14 0m7 7v4m-4 0h8M12 1a3 3 0 00-3 3v7a3 3 0 006 0V4a3 3 0 00-3-3z" /></svg>
          Record with AI Note Taker
        </button>
      )}

      <div className="flex items-center justify-end mb-2">
        <button onClick={handleLoadPrevious} disabled={loadingPrev}
          className="text-xs text-brand-600 hover:text-brand-700 font-medium disabled:opacity-50">
          {loadingPrev ? 'Loading...' : 'Load previous note'}
        </button>
      </div>
      {[
